...
public:
	...
	Matrix4x4SSE &_vectorcall LookAtLH(const Vec4SSE &EyePosition, const Vec4SSE &FocusPosition,
		const Vec4SSE &UpDirection);
	// Builds a view matrix for a right-handed coordinate system.  Angles the camera
	// to point at the specified coordinates.
	Matrix4x4SSE &_vectorcall LookAtRH(const Vec4SSE &EyePosition, const Vec4SSE &FocusPosition,
		const Vec4SSE &UpDirection);

	// Builds a view matrix for a left-handed coordinate system.  Angles the camera
	// to point in the specified direction.
	Matrix4x4SSE &_vectorcall LookToLH(const Vec4SSE &EyePosition, const Vec4SSE &EyeDirection,
		const Vec4SSE &UpDirection);
	// Builds a view matrix for a right-handed coordinate system.  Angles the camera
	// to point in the specified direction.
	Matrix4x4SSE &_vectorcall LookToRH(const Vec4SSE &EyePosition, const Vec4SSE &EyeDirection,
		const Vec4SSE &UpDirection);

	// Builds an orthogonal projection matrix for left-handed coordinate system
	Matrix4x4SSE &_vectorcall OrthographicsLH(const float ViewWidth, const float ViewHeight,
		const float NearZ, const float FarZ);
	// Builds an orthogonal projection matrix for right-handed coordinate system
	Matrix4x4SSE &_vectorcall OrthographicsRH(const float ViewWidth, const float ViewHeight,
		const float NearZ, const float FarZ);

	// Builds a custom orthogonal projection matrix for a left-handed coordinate system.
	Matrix4x4SSE &_vectorcall OrthographicOffCenterLH(const float ViewLeft, const float ViewRight,
	...
	Matrix4x4SSE &_vectorcall OrthographicOffCenterRH(const float ViewLeft, const float ViewRight,
		const float ViewBottom, const float ViewTop, const float NearZ,
		const float FarZ);

	// Builds a left-handed perspective projection matrix based on a field of view.
	Matrix4x4SSE &_vectorcall PerspectiveFovLH(const float FovAngleY, const float AspectRatio,
		const float NearZ, const float FarZ);
	// Builds a right-handed perspective projection matrix based on a field of view.
	Matrix4x4SSE &_vectorcall PerspectiveFovRH(const float FovAngleY, const float AspectRatio,
		const float NearZ, const float FarZ);

	// Builds a left-handed perspective projection matrix.
	Matrix4x4SSE &_vectorcall PerspectiveLH(const float ViewWidth, const float ViewHeight,
		const float NearZ, const float FarZ);
	// Builds a right-handed perspective projection matrix.
	Matrix4x4SSE &_vectorcall PerspectiveRH(const float ViewWidth, const float ViewHeight,
		const float NearZ, const float FarZ);

	// Builds a custom version of a left-handed perspective projection matrix.
	Matrix4x4SSE &_vectorcall PerspectiveOffCenterLH(const float ViewLeft, const float ViewRight,
		const float ViewBottom, const float ViewTop, const float NearZ,
		const float FarZ);
	// Builds a custom version of a right-handed perspective projection matrix.
	Matrix4x4SSE &_vectorcall PerspectiveOffCenterRH(const float ViewLeft, const float ViewRight,
		const float ViewBottom, const float ViewTop, const float NearZ,
		const float FarZ);
	...
	Matrix4x4SSE &_vectorcall RotateQuaternion(const Vec4SSE &Quaternion);
